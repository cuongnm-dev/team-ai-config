...
14. BATCH RUN — TREO MÁY CHẠY HẾT MODULES
                        14.1 3 path                          14.2 Cursor Background Agent
                        14.3 Manual chain                    14.4 Skill /run-all-modules (proposed)
                        14.5 Khuyến nghị + chi phí
```
...
## 14. Batch run — treo máy chạy hết modules cho đến xong

Project lớn (20+ modules × 30 phút – 4 giờ mỗi module ≈ tổng 40+ giờ) không thể ngồi đợi từng `/resume-module` một. Có 3 path:

### 14.1 Tổng quan 3 path

| Path | Setup | Máy local | Cost | Phù hợp |
|---|---|---|---|---|
| 1. Cursor Background Agent (cloud) | Zero | Có thể tắt | ~$50-200 / 20 modules (Cursor Pro tier) | Project urgent, cần xong qua đêm |
| 2. Manual chain trong 1 chat | Zero | Phải bật + Cursor chạy nền | Theo plan Cursor đang dùng | Test 1-2 lần, xem skill có đủ ổn cho 20 module |
| 3. Skill `/run-all-modules` (chưa có) | Cần build skill | Tuỳ skill chạy local hay schedule | Như path 1 | Run định kỳ, project lớn, nhiều dev |

### 14.2 Path 1 — Cursor Background Agent (khuyến nghị nhất)

Cursor có **Background Agent** chạy cloud-side, máy local tắt cũng OK.

```
1. Mở Cursor → bật Background Agent (icon đám mây ở góc)
2. Submit prompt:
   "Chạy /resume-module lần lượt cho TẤT CẢ modules theo thứ tự topological
    của module-catalog.json depends_on. Mỗi module xong mới qua module tiếp.
    Quy tắc:
      - Module status=done → skip
      - Fail HARD blocked → STOP, báo lại
      - Fail user-needed → log vào _batch-log.md + tiếp tục module sau
    Cuối cùng: in tổng kết bao nhiêu module done / failed / skipped."
3. Tắt laptop, đi ngủ
4. Sáng mai mở Cursor → xem kết quả + xử lý các module fail nếu có
```

**Ưu**: không setup, hoạt động ngay, máy local không cần bật.
**Nhược**: tiêu thụ Cursor cloud quota (giá theo plan).

### 14.3 Path 2 — Manual chain trong 1 chat (đơn giản nhất, dùng máy local)

Trong chat Cursor, list cả 20 lệnh:

```
Chạy lần lượt, đợi từng cái xong mới qua:

/resume-module M-001-identity-access
/resume-module M-002-taxpayer-360-mdm
/resume-module M-003-registration-lifecycle
... (20 modules)
/resume-module M-020-platform-ops-devsecops

Quy tắc:
- Module status=done → skip
- Fail HARD → dừng + báo lại
- Fail user-needed → log + tiếp tục module sau
```

Cursor agent sẽ execute tuần tự. **Để máy bật + Cursor mở liên tục** cho đến khi xong.

**Ưu**: không setup gì.
**Nhược**: máy local phải bật suốt; mất kết nối → đứt chuỗi; nếu Cursor reload context → có thể mất tiến độ.

### 14.4 Path 3 — Skill `/run-all-modules` (đề xuất, chưa build)

Skill batch wrapper, design dự kiến:

```bash
/run-all-modules                                # tất cả modules status ∈ {proposed, in-progress, blocked}
/run-all-modules --from M-005                   # bắt đầu từ M-005
/run-all-modules --status proposed              # chỉ modules chưa start
/run-all-modules --on-error stop|skip|continue  # policy mặc định: skip
/run-all-modules --dry-run                      # in thứ tự sẽ chạy, không thực thi
```

**Cơ chế**:
1. Đọc `module-catalog.json` + topological sort theo `depends_on`
2. Lưu state `docs/intel/_batch-state.json`:
   ```json
   {
     "started_at": "...",
     "queue": ["M-005", "M-006", ...],
   ...
   ```
3. Loop: `Skill(resume-module, {next-id})` → đợi verdict → log → next
4. Resume-able: nếu interrupt → re-invoke `/run-all-modules` đọc state, tiếp tục
5. Final report: số module done / failed / skipped, tổng token usage, thời gian

**Effort build**: ~250 dòng SKILL.md + tích hợp Task() result của resume-module. Tracker tại GitHub issue (TODO).

### 14.5 Khuyến nghị + chi phí

| Hoàn cảnh | Path nên chọn |
|---|---|
| Project test, 1-2 module trước | 2 (manual chain ngắn) |
| Project urgent, cần xong qua đêm | **1 (Background Agent)** |
| Run định kỳ / project lớn / team nhiều người | 3 (build skill khi có team bandwidth) |

**Ước tính chi phí cho 20 modules** (Path L, ~2h mỗi module):
- Tokens: ~50K-200K input + 30K-80K output mỗi module → tổng 1M-4M token
- Cost (Sonnet): ~$3-12/M input + $15/M output → ~**$50-200 cho 20 modules**
- Cost (Opus): ~3-4× higher → ~$150-600
- Thời gian: 20 × 2h ≈ 40h wall-clock (chia parallel modules không phụ thuộc nhau xuống ~10-20h)

**Tip giảm chi phí**:
- Set `output-mode: lean` (default) cho mọi module
- Modules Path S nếu phù hợp → token thấp hơn nhiều
- Defer "estimated" features (chọn option 2 trong scaffold prompt)
- Nếu intel rich (doc-brief đầy đủ) → BA stage chạy nhanh, ít rework

**Track tiến độ**:
- `cat docs/intel/module-catalog.json | jq '[.modules[].status] | group_by(.) | map({(.[0]): length}) | add'` xem stats
- Module nào fail → review `docs/modules/M-NNN/_state.md.frontmatter.blocked-by` rồi quyết định fix manual hoặc rerun
