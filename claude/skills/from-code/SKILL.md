...
**Phase 5+ MUST call `Bash("ai-kit sdlc ...")` scaffolding commands instead of `Write`/`mkdir`/glob** per ADR-005 D3 (supersedes prior CD-8 v3 MCP wording).

| Legacy step | New ai-kit CLI command |
|---|---|
| Write `_state.md` per feature | `ai-kit sdlc scaffold module --id M-NNN --name "..." --slug ...` + `ai-kit sdlc scaffold feature --module M-NNN --id F-NNN --name "..." --slug ...` |
| Write `feature-brief.md` | Rendered atomically by `ai-kit sdlc scaffold module` from JS template literal (bundled) |
| Update `feature-catalog.json` | Updated atomically by `ai-kit sdlc scaffold feature` |
| Update `module-catalog.json` (NEW per ADR-003) | Created/updated by `ai-kit sdlc scaffold module/feature` |
| Phase 5 ID allocation glob | `ai-kit sdlc scaffold module` atomic + verify scope `id_uniqueness` |
| Phase 8 quality gate | `verify(scopes=['all'], strict_mode='block')` |

**Auto-detect implementations** (Phase 5 enrichment): code-harvester emits per-feature `implementations.yaml.primary` (path) + consumers list — write via `update_state(op=field)`.

**Forbidden after migration**:
- ❌ `Write` for any scaffolding file in `docs/{modules,features,hotfixes}/**`
- ❌ Glob `{features-root}/F-*/` for ID allocation
- ✅ Read code source files (apps/services/libs/{src,internal,cmd}/**) — unchanged

**Reference**: `D:\AI-Platform\maintainer-notes\adr\ADR-003-sdlc-2tier-module-feature.md` D8/D11.
...
# Resume Protocol Lockout escape flags (audit-2026-05-06 T1-7):
/from-code --reset-gate <0|a|b>         # reset confirmed gate; backup to .bak.{ISO}; re-enter gate loop
/from-code --rerun-phase P{N}           # reset all phases from N onwards; archive dependent artifacts
/from-code --reset-all                  # move docs/intel/ → .bak.{ISO}, delete _pipeline-state.json
```

Default: full pipeline, bridge-ready, auto-invoke `/generate-docs` prompt at P8 (user chooses).

**Resume Protocol Lockout escape**: per audit-2026-05-06 T1-7 + the same flag set documented in `~/.claude/skills/from-doc/SKILL.md` § Step 0. Skill-side state mutation pending MCP `update_state(op=stage_rollback)` primitive (T1-12 backend roadmap). On `--reset-gate` or `--rerun-phase`:
1. Read `_pipeline-state.json`
2. Backup `_pipeline-state.json.bak.{ISO}`
3. Mutate state (gate.confirmed=false / phases[N..end].status=pending)
4. Move dependent artifacts to `docs/intel/_history/{ISO}/`
5. Print confirmation + EXIT (user re-invokes without flag to continue from reset point)

**Quality regression guard**: `--reset-gate` after `iterations > 0` prompts user with prior metrics (feature-count, blocking-gaps) before confirming reset.
