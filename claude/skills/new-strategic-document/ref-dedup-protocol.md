...
**MCP unavailable → BLOCK pipeline** per CLAUDE.md CD-8 v3 (no silent local fallback). User chạy `docker compose up -d` từ `~/.ai-kit/ai-kit/mcp/etc-platform/` trước khi retry. (Audit-2026-05-06 T2-aligned: previous "local file fallback" wording contradicted CD-8 v3.)
