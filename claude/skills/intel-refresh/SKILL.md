...
**Drift detection MUST use `Bash("ai-kit sdlc verify --workspace . --scopes freshness,cross_references,id_uniqueness")`** instead of manual hash check + glob walk. Parse stdout JSON.

| Legacy step | New ai-kit CLI command |
|---|---|
| Read `_meta.json` + checksum vs sources | `ai-kit sdlc verify --scopes freshness` returns TTL exceeded + content-hash mismatch |
| Re-emit catalogs via Write | `ai-kit sdlc state update --op field` for incremental updates; `ai-kit sdlc scaffold *` only for NEW entities |
| Detect orphan folders | `ai-kit sdlc verify --scopes id_uniqueness` returns filesystem-orphan + catalog-orphan (HIGH severity) |
| Cleanup `.tmp` files | `ai-kit sdlc autofix --fix-classes orphan-removal --no-dry-run --confirm-destructive` |

After verify findings:
- HIGH severity → `autofix` for fixable classes
- MEDIUM → flag to user, proceed
- LOW → log only

**Reference**: ADR-003 D7 + D10-6.
