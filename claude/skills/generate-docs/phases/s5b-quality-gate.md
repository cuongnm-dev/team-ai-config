# Stage 5b — Quality Gate (6 passes)

**Stage**: 5 QUALITY, sub-step 5b
...
### Pass 7 — Confidence audit (NEW)

Read `{DOCS_PATH}/intel/validation-report.json.confidence_stats` (produced by `intel-validator`).

```
total_low = sum(stats[artifact].low for artifact in stats)
total_entries = sum(sum(stats[artifact].values()) for artifact in stats)
low_ratio = total_low / total_entries

if confidence_stats.low_confidence_critical:
    blocker — list each entry, escalate via Gate B
elif low_ratio > 0.05:                  # > 5% low
    major — recommend producer re-run with --rerun-stage <N>
elif low_ratio > 0.02:                  # 2-5% low
    minor — log to expansion-gaps.md
```

Fix via: re-run `tdoc-researcher` with `scope=stage1|stage2`, OR collect missing info via `manual-interview` agent → re-merge with `intel-merger`.

See `notepads/confidence-routing.md` for per-consumer routing rules.
