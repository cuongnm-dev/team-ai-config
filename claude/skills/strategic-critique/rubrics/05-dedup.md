...
**MCP-aware audit (Phase 3+, per CLAUDE.md MCP-2)**: nếu Đề án được build trên MCP-enabled pipeline, mỗi proposal đáng lẽ đã đi qua `dedup_check`. Reviewer cần verify:
1. `mcp__etc-platform__dedup_check({problem, solution_summary})` cho từng giải pháp Section 5/6 — nếu `exact_match` hoặc `similar` ≥ 0.7 → MAJOR finding (đã có giải pháp tương tự, vẫn đề xuất build mới).
2. Cross-check với KB: `mcp__etc-platform__kb_query(domain="ecosystem")` — đối chiếu catalog dưới đây với KB entries fresh.
3. Khi MCP không khả dụng (offline review): dùng catalog static dưới đây làm baseline.
