...
**Output language**: Vietnamese cho findings. English cho agent instructions.

**Role**: Agent đóng vai **thẩm định cấp Bộ/Tỉnh** đọc Đề án researcher vừa viết, tìm lỗi mà thẩm định thật sẽ raise. Mục tiêu: researcher fix trước khi nộp, không phải AI viết thay.

---

## Khi nào dùng

| Tình huống | Dùng `/strategic-critique`? |
|---|---|
| Researcher vừa viết xong draft Đề án | ✅ Có |
| Researcher chưa có draft, muốn AI viết hộ | ❌ Không — dùng `/new-strategic-document` (nhưng output sẽ filler, xem warning skill đó) |
| Chuẩn bị nộp thẩm định | ✅ Critical — chạy 1-2 lần để catch issues |
| Đã được phản hồi thẩm định, muốn check đã fix hết | ✅ Chạy lại |
...
```
/strategic-critique <đường-dẫn-file>              # critique toàn bộ
/strategic-critique <đường-dẫn> --section 3,5     # chỉ sections đó
/strategic-critique <đường-dẫn> --severity blocker,major  # chỉ issue nặng
/strategic-critique <đường-dẫn> --check dedup,legal,vague # chỉ 1 vài check
```

**Input formats**: `.docx`, `.md`, `.pdf` (dùng `anthropic-skills:docx|pdf` để parse)
...
- `.docx` → extract text + tables + heading structure via `anthropic-skills:docx`
- `.md` → read trực tiếp
- `.pdf` → OCR qua `anthropic-skills:pdf` (nếu scan) hoặc parse direct

Normalize thành structure:
```yaml
document:
  total_pages: 45
  sections:
...
Mỗi check load rubric tương ứng + apply:

1. **Formal check** — outline NĐ 30, section numbering, bảng/hình format
2. **Substantive check** — có số liệu cụ thể, tên đơn vị, mốc thời gian
3. **Coherence check** — objectives ↔ problems ↔ solutions ↔ budget cohesive
4. **Alignment check** — fit QĐ 749 KPIs, CT 34 9 nguyên tắc
5. **DEDUP check** — giải pháp có trùng NDXP / LGSP / CSDLQG / Gov Cloud có sẵn không
6. **Feasibility check** — timeline realistic, budget realistic vs TT 04/2020
7. **Legal ref validity** — văn bản cited còn hiệu lực, điều khoản đúng

Chi tiết rubric trong `rubrics/*.md`.
