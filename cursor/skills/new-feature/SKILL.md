...
Absorbs: update-feature.
Single skill for new pipeline, resume existing, or update completed feature.
User-facing output: Vietnamese. Artifact files and dispatcher prompts: English.

---

## Step 1 — Detect: New or Resume?
...
| Result | Action |
|---|---|
| Found + `status` ≠ `done` | → **RESUME FLOW** (Step 5) |
| Found + `status: done` | → **UPDATE FLOW** (Step 5U) |
| Not found | → **NEW FLOW** (Step 2) |

No `feature-id` provided → **NEW FLOW** (Step 2).
...
```jsonc
{
  ...
  "priority": "{critical|high|medium|low}",
  "role_visibility": [
    // Pre-populate from Step 4 "Roles thấy được" answer
  ...
  "test_case_ids": [],
  "test_evidence_ref": null,                  // qa stage will populate via close-feature
  "implementation_evidence": null,            // close-feature populates
...
// Append into features[] (or modules[].features[] per existing structure):

{
  "feature_id": "{feature-id}",
  "module": "{module from Step 4}",
  "status": "planned",
  "planned_routes": [],         // empty — sa will fill routes[] at sa stage
  "source_producers": ["new-feature"],
  "confidence": "low"
}
```
...
//   Append into permissions[]:

{
  "id": "P-{feature-id}-{role-slug}-read",
  "role_slug": "{role-slug}",
  "resource": "feature:{feature-id}",
  "action": "read",                  // proposed default; sa will refine to {read,write,approve,export}
  "status": "proposed",              // sa upgrades to "active" with concrete action
  "confidence": "low",
  "source_producers": ["new-feature"],
  "evidence": [{
    "kind": "manual",
...
>
> **Why pre-register at all**: if user runs `/generate-docs` between `/new-feature` and `sa` stage (vd: preview navigation map), the feature would be invisible without these placeholders. sitemap navigation tables + permission audit reports stay consistent across the lifecycle.

---

### Step 4.7 — Exit-gate verification (LIFECYCLE.md §5.1)

Before proceeding to Step 6 (Dispatcher), verify all EXIT-GATES of /new-feature contract are satisfied:
...
→ Proceed to **Step 6 — Dispatcher Loop**

---

## RESUME FLOW

### Step 5 — Read state and report checkpoint

Read full `_state.md`. Extract:
- `docs-path`, `current-stage`, `stages-queue`, `completed-stages`
- Active Blockers, Next Action sections

**If `current-stage` is `dev-wave-{N}` or `fe-dev-wave-{N}`** (mid-wave):
- Read `{docs-path}/04-tech-lead-plan.md` → get all tasks for wave N
...
If `_state.md` lacks `stages-queue` (legacy pipeline):
- Reconstruct: `stages-queue` = [all path stages] minus [completed-stages keys]
- Update `_state.md` before continuing

**If `clarification-notes:` is non-empty** (PM asked a question before pausing):
- Show user: `"PM hỏi trước khi dừng: {clarification-notes}"`
- Ask user to provide answer
- Append user's answer to `clarification-notes` field:
  ```yaml
  clarification-notes: |
    PM question: {original question}
    User answer: {user's response}
  ```
- Dispatcher will inject this into the next agent's prompt, then clear the field
...
- [ ] `stages-queue` set (new) or reconstructed (resume)
- [ ] `feature-map.yaml` updated with new/resumed feature entry
- [ ] Resume: do NOT re-run stages that already have a verdict

---

## UPDATE FLOW (absorbed from update-feature)
