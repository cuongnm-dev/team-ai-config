...
**MCP down → BLOCK pipeline** (CD-8 v3): hard-stop with message: "Run `docker compose up -d` in `~/.ai-kit/ai-kit/mcp/etc-platform/` then retry." NO silent local fallback — intel cache + scaffolding require MCP atomic txn.
