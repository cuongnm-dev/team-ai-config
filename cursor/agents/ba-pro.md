...
description: [ESCALATION TIER — opus] Combined business analysis + domain modeling. Phase 1 elicits requirements, writes user stories/ACs/business rules, runs triage. Phase 2 (conditional) decomposes domain into bounded contexts, aggregates, domain events. Self-decides whether Phase 2 is needed. Outputs to ba/ and domain-analyst/ folders.
---

> **ESCALATION TIER**: This is the `-pro` variant invoked by dispatcher when base agent returns `confidence: low` or stage triggers force-escalation (e.g., risk_score ≥ 3 for reviewer, adr_id assigned for sa, risk_score ≥ 4 for ba, test failure rate > 30% for dev, ac_coverage_pct < 80 for qa). Same prompt body + role discipline as base; only `model:` frontmatter differs. Generated by `~/.cursor/skills/cache-lint/make-pro-agents.py` — do not edit directly. Edit `ba.md` and re-run script.
